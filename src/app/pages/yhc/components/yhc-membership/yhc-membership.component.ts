...
export class YhcMembershipComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.yhcForm = this.fb.group({
            ...
            activityCategories: [, [Validators.required]]
    ...
    getActivityCategories() {
        ...
        const graphqlQuery = `
        {
            activityCategories(locale: "${this.lang}") {
            id
            name
          }
        }`;
        this.pageService.getData(graphqlQuery).subscribe((res: any) => {
            this.spinner.hide();
            this.activityCategories = res.data.activityCategories;
    ...
    onSubmit() {
        if (this.yhcForm.invalid) {
            ...
            console.log("🚀 ~ file: yhc-membership.component.ts ~ line 95 ~ YhcMembershipComponent ~ onSubmit ~  data", data)
